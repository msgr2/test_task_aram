...
namespace App\Services\SendingProcess\Telecom\Transport;

use ArrayIterator;
use backend\models\GatewaysErrors;
use App\Services\SendingProcess\Telecom\Transport\Exception\SocketTransportException;
...
class SocketTransport
{
     ...
     */
    public function __construct(array $hosts, mixed $ports, protected $persist = false, mixed $debugHandler = null)
    {
        ...
        foreach ($hosts as $host) {
            ...
            if (preg_match('/^([12]?[0-9]?[0-9]\.){3}([12]?[0-9]?[0-9])$/', $hostname)) {
                ...
                if (!self::$forceIpv4) {
                    ...
                    if ($records === false && $this->debug) call_user_func($this->debugHandler,
                        'DNS lookup for AAAA records for: ' . $hostname . ' failed');
                    if ($records) {
                    ...
                    if ($this->debug) call_user_func($this->debugHandler,
                        "IPv6 addresses for $hostname: " . implode(', ', $ip6s));
                }
                if (!self::$forceIpv6) {
                    // if not in IPv6 mode check the A records also
                    $records = dns_get_record($hostname, DNS_A);
                    if ($records === false && $this->debug) call_user_func($this->debugHandler,
                        'DNS lookup for A records for: ' . $hostname . ' failed');
                    if ($records) {
                        foreach ($records as $r) {
                    ...
                    if ($this->debug) call_user_func($this->debugHandler,
                        "IPv4 addresses for $hostname: " . implode(', ', $ip4s));
                }
            }
     ...
     * @param integer $lvl
     */
    public function setSocketOption($option, mixed $value, $lvl = SOL_SOCKET)
    {
    ...
    public function isOpen()
    {
        if (!($this->socket instanceof Socket)) {
            if ($this->debug) {
                call_user_func($this->debugHandler, "socket is not instanceof \Socket");
            }
            return false;
        }
        $r = null;
        ...
        if ($res === false) throw new SocketTransportException('Could not examine socket; ' .
            socket_strerror(socket_last_error()), socket_last_error());
        if (!empty($e)) {
            if ($this->debug) {
                call_user_func($this->debugHandler, "socket_last_error: " . socket_last_error());
            }
    ...
    public function open()
    {
        if (!self::$forceIpv4) {
            ...
            if ($socket6 == false) throw new SocketTransportException('Could not create socket; ' .
                socket_strerror(socket_last_error()), socket_last_error());
        ...
        if (!self::$forceIpv6) {
            ...
            if ($socket4 == false) throw new SocketTransportException('Could not create socket; ' .
                socket_strerror(socket_last_error()), socket_last_error());
    ...
    public function read($length)
    {
        ...
        if ($d === false) throw new SocketTransportException('Could not read ' . $length . ' bytes from socket; ' .
            socket_strerror(socket_last_error()), socket_last_error());
        if ($d === '') return false;
        return $d;
    }
    ...
    public function readAll($length)
    {
        ...
        while ($r < $length) {
            ...
            if ($r === false) throw new SocketTransportException('Could not read ' . $length . ' bytes from socket; ' .
                socket_strerror(socket_last_error()), socket_last_error());
            $d .= $buf;
            ...
            if ($res === false) throw new SocketTransportException('Could not examine socket; ' .
                socket_strerror(socket_last_error()), socket_last_error());
            if (!empty($e)) throw new SocketTransportException('Socket exception while waiting for data; ' .
                socket_strerror(socket_last_error()), socket_last_error());
    ...
    public function write($buffer, $length)
    {
        ...
        while ($r > 0) {
            ...
            if ($wrote === false) throw new SocketTransportException('Could not write ' . $length .
                ' bytes to socket; ' . socket_strerror(socket_last_error()), socket_last_error());
            $r -= $wrote;
            if ($r == 0) return;
            ...
            if ($res === false) throw new SocketTransportException('Could not examine socket; ' .
                socket_strerror(socket_last_error()), socket_last_error());
            if (!empty($e)) throw new SocketTransportException('Socket exception while waiting to write data; ' .
                socket_strerror(socket_last_error()), socket_last_error());
